...
class cpptoswift
{
	...
	func parse() throws
	{
		let dfstr = NSString(string:destinationFile)
		...
		let	shortenedDestinationFile = dfstr.substringFromIndex( slashRange!.location + slashRange!.length )

		let sfstr = NSString(string:sourceFile)
		let sslashRange : NSRange? = sfstr.rangeOfString( "/", options: NSStringCompareOptions.BackwardsSearch, range: NSRange(location:0,length:sfstr.length), locale: nil )
		let	shortenedSourceFile = sfstr.substringFromIndex( sslashRange!.location + sslashRange!.length )
